    ...
    public class ReleaseRepository : BasicRepository<AlbumRelease>, IReleaseRepository
    {
        ...
        public List<AlbumRelease> FindByAlbum(int id)
        {
            ...
            var builder = new SqlBuilder()
                .Join<AlbumRelease, Book>((r, a) => r.AlbumId == a.Id)
                .Join<Book, AuthorMetadata>((a, m) => a.AuthorMetadataId == m.Id)
                .Where<AlbumRelease>(r => r.AlbumId == id);

            return _database.QueryJoined<AlbumRelease, Book, AuthorMetadata>(builder, (release, album, metadata) =>
                    {
                        release.Album = album;
                        release.Album.Value.AuthorMetadata = metadata;
                        return release;
                    }).ToList();
        }
